...
CFLAGS  := -g$(DBGLVL) -O$(OPTLVL) -Wall -std=$(CXXSTD) -pthread $(DEFINES) $(INCDIRS)
LIBS    := -lpthread -lutil
LDFLAGS := -g$(DBGLVL) $(LIBDIRS)
...
.PHONY install:
install: all
		$(INSTALL) -m 755 MMDVMHost $(BINDIR)/
		$(INSTALL) -m 755 RemoteCommand $(BINDIR)/

.PHONY install-service:
install-service: install $(CONFDIR)/MMDVM.ini
		@useradd --user-group -M --system mmdvm --shell /bin/false || true
		@usermod --groups dialout --append mmdvm || true
		@mkdir $(LOGDIR) || true
		@chown mmdvm:mmdvm $(LOGDIR)
		@cp ./linux/systemd/mmdvmhost.service /lib/systemd/system/
		@systemctl enable mmdvmhost.service

$(CONFDIR)/MMDVM.ini:
		@cp -n MMDVM.ini $(CONFDIR)/MMDVM.ini
		@sed -i 's/FilePath=./FilePath=\/var\/log\/mmdvm\//' $(CONFDIR)/MMDVM.ini
		@sed -i 's/Daemon=0/Daemon=1/' $(CONFDIR)/MMDVM.ini
		@chown mmdvm:mmdvm $(CONFDIR)/MMDVM.ini

.PHONY uninstall-service:
uninstall-service:
		@systemctl stop mmdvmhost.service || true
		@systemctl disable mmdvmhost.service || true
		@rm -f $(BINDIR)/MMDVMHost || true
		@rm -f /lib/systemd/system/mmdvmhost.service || true

clean:
		$(RM) -f MMDVMHost RemoteCommand *.o *.d *.bak *~ GitVersion.h
